...
export interface DaypartSales {
  daypart: string; // e.g. "morning", "lunch", "afternoon"
  hourStart: number;
  hourEnd: number;
  orderCount: number;
  netSales: number;
...
export interface LaborSummary {
  ...
  laborPercent: number; // as decimal, e.g. 0.32
  /** Breakdown by daypart if available */
...
export interface RecipeStatus {
  ...
  missingItems: { itemGuid: string; itemName: string; menuPrice: number }[];
  source: 'computed';
  fetchedAt: string;
  /**
   * NOTE: MarginEdge API does not expose recipe data directly.
   * This model is populated by cross-referencing Toast menu items
   * against MarginEdge product mappings. Items without a matching
   * MarginEdge product are flagged as missing recipe/cost mapping.
   */
}
...
export interface DiscountSummary {
  ...
  discountPercent: number; // as decimal vs net sales
...
export interface InventoryCountStatus {
  ...
  fetchedAt: string;
  /**
   * NOTE: MarginEdge API does not expose inventory count data.
   * This field must be populated from manual input or future API access.
   */
}

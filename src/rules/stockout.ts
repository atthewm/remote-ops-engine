...
export class StockoutRule implements RuleHandler {
  ...
  async evaluate(storeId: string, config: AppConfig): Promise<RuleResult> {
    ...
    let recentMix: ItemMix | null = null;
    try {
    ...
    try {
      ...
      for (const product of products) {
        const cost = typeof (product as Record<string, unknown>).cost === 'number'
          ? (product as Record<string, unknown>).cost as number
          : typeof (product as Record<string, unknown>).price === 'number'
            ? (product as Record<string, unknown>).price as number
            : 0;
    ...
    const totalRevenueLoss = significantItems.reduce(
      (sum, item) => sum + item.estimatedDailyRevenueLoss,
      0,
    );
    ...
    const fingerprint = generateFingerprint(
      ...
      storeId,
      asOfDate,
      `disabled_${significantItems.length}`,
    );

    const whatParts: string[] = [];
    whatParts.push(
      `${significantItems.length} significant menu items are currently disabled or hidden.`,
    );
    whatParts.push(
      `Total disabled items: ${stockoutResult.items.length}. Significant items (high margin, high velocity, or material revenue impact): ${significantItems.length}.`,
    );

    if (totalRevenueLoss > 0) {
      whatParts.push(
        `Estimated daily revenue loss: $${totalRevenueLoss.toFixed(2)}.`,
      );
    }
...
function buildRecommendedAction(
  ...
  if (disabledItems.length > 0) {
    actions.push(
      `Re enable ${disabledItems.length} disabled items in Toast if stock is available.`,
    );
  }

  if (eightySixedItems.length > 0) {
    actions.push(
      `Check inventory for ${eightySixedItems.length} 86'd items and restore when restocked.`,
    );
  }

  if (hiddenItems.length > 0) {
    actions.push(
      `Review ${hiddenItems.length} hidden items to confirm they should remain hidden.`,
    );
  }

  if (totalRevenueLoss >= 100) {
    actions.push(
      `Estimated daily impact is $${totalRevenueLoss.toFixed(2)}. Prioritize restoring highest revenue items first.`,
    );
  }

...
export class DiscountCompRule implements RuleHandler {
  ...
  async evaluate(storeId: string, config: AppConfig): Promise<RuleResult> {
    ...
    let metrics: ExceptionMetrics | null = null;
    ...
    let trailingVrc: VoidsRefundsComps | null = null;
    let trailingDiscounts: DiscountSummary | null = null;
    try {
    ...
    evaluateMetric(
      ...
      thresholds.discountPercentYellow,
      thresholds.discountPercentRed,
      discounts.totalDiscounts,
      trailingDiscounts?.discountPercent ?? null,
      thresholds.trailingSpikeMultiplier,
    );
    ...
    evaluateMetric(
      ...
      'Voids',
      vrc.voidPercent,
      thresholds.voidPercentYellow,
      thresholds.voidPercentRed,
      vrc.voidAmount,
      trailingVrc?.voidPercent ?? null,
      thresholds.trailingSpikeMultiplier,
    );
    ...
    evaluateMetric(
      ...
      'Comps',
      vrc.compPercent,
      thresholds.compPercentYellow,
      thresholds.compPercentRed,
      vrc.compAmount,
      trailingVrc?.compPercent ?? null,
      thresholds.trailingSpikeMultiplier,
    );
    ...
    evaluateMetric(
      ...
      'Refunds',
      vrc.refundPercent,
      thresholds.refundPercentYellow,
      thresholds.refundPercentRed,
      vrc.refundAmount,
      trailingVrc?.refundPercent ?? null,
      thresholds.trailingSpikeMultiplier,
    );
    ...
    evaluateMetric(
      breaches,
      'Total Exceptions',
      vrc.totalExceptionPercent + discounts.discountPercent,
      thresholds.totalExceptionPercentYellow,
      thresholds.totalExceptionPercentRed,
      vrc.totalExceptionAmount + discounts.totalDiscounts,
      trailingVrc && trailingDiscounts
    ...
    const severity: Severity = breaches.some(b => b.severity === 'red') ? 'red' : 'yellow';
    ...
    // Breakdown section
    whatParts.push(
      `Voids: ${vrc.voidCount} totaling $${vrc.voidAmount.toFixed(2)} (${pct(vrc.voidPercent)} of sales).`,
    );
    whatParts.push(
      `Refunds: ${vrc.refundCount} totaling $${vrc.refundAmount.toFixed(2)} (${pct(vrc.refundPercent)} of sales).`,
    );
    whatParts.push(
      `Comps: ${vrc.compCount} totaling $${vrc.compAmount.toFixed(2)} (${pct(vrc.compPercent)} of sales).`,
    );
    whatParts.push(
      `Discounts: ${discounts.discountCount} totaling $${discounts.totalDiscounts.toFixed(2)} (${pct(discounts.discountPercent)} of sales).`,
    );
    ...
    if (spikeBreaches.length > 0) {
      whatParts.push(
        `Trailing average spikes detected: ${spikeBreaches.map(b => b.metric).join(', ')}.`,
      );
    }
   ...
   */
  private async fetchMetrics(storeId: string, businessDate: string): Promise<ExceptionMetrics> {
   ...
   */
  private async fetchTrailingAverage(
    storeId: string,
    now: Date,
    days: number,
  ): Promise<{ vrc: VoidsRefundsComps; discounts: DiscountSummary }> {
    const trailingMetrics: ExceptionMetrics[] = [];
    ...
    const avgVrc: VoidsRefundsComps = {
      storeId,
    ...
    const avgDiscounts: DiscountSummary = {
      storeId,
      businessDate: 'trailing_avg',

 ...
 */
export function formatAlert(alert: NotificationEvent): string {
 ...
 */
export function formatDailyOpsDigest(digest: AlertDigest): string {
 ...
 */
export function formatWeeklyExecSummary(summary: WeeklyExecutiveSummary): string {
 ...
 */
export function buildRevenueTrendHtml(
  labels: string[],
  revenues: number[],
): string {
  if (labels.length < 2 || revenues.length < 2) return '';
 ...
 * Returns empty string if insufficient data.
 */
export function buildDriveThruTrendHtml(
  labels: string[],
  avgSeconds: number[],
): string {
  if (labels.length < 2 || avgSeconds.length < 2) return '';
 ...
 */
function formatMetricsTable(metrics: Record<string, string | number>): string {
  const entries = Object.entries(metrics).filter(
    ([key]) => !key.startsWith('_chart'),
  );
  if (entries.length === 0) return '';

  const rows = entries
    .map(([key, value]) =>
      `<tr><td style="padding: 2px 8px;"><b>${escapeHtml(key)}</b></td>` +
 ...
 */
function formatDigestSection(section: DigestSection): string {
  ...
  const rows = metrics
    .map(([key, value]) =>
      `<tr><td style="padding: 2px 8px;">${escapeHtml(key)}</td>` +
      `<td style="padding: 2px 8px;">${escapeHtml(String(value))}</td></tr>`
    )
    .join('\n');
 ...
 */
function formatAlertSummary(alerts: NotificationEvent[]): string {
  if (alerts.length === 0) {
 ...
 */
function formatActionItems(alerts: NotificationEvent[]): string {
 ...
 */
function formatOwnerScorecard(
  entries: WeeklyExecutiveSummary['ownerScorecard'],
): string {
  if (entries.length === 0) {

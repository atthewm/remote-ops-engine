...
import type { NotificationEvent, Audience, Severity } from '../models/normalized.js';
...
export interface RoutingDecision {
  ...
  channels: { channelKey: string; channelId: string; teamId: string }[];
...
export function routeAlert(
  ...
  const targetChannels: { channelKey: string; channelId: string; teamId: string }[] = [];
 ...
 */
function resolveChannelKeys(audience: Audience, config: AppConfig): string[] {
 ...
 */
function isExecEligible(alert: NotificationEvent): boolean {
  if (alert.severity === 'red') return true;
  ...
  if (execSummaryRules.includes(alert.ruleId)) return true;
 ...
 */
function resolvePlanId(config: AppConfig): string | null {

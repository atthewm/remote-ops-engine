...
import type {
  ...
  DiscountEntry,
  VoidsRefundsComps,
  StockoutOrDisabledItem,
  StockoutEntry,
} from '../models/normalized.js';

// ─── Raw response shapes from Toast MCP ───
...
interface RawSelection {
  guid?: string;
  itemGuid?: string;
  item?: { guid?: string; name?: string; entityType?: string };
  displayName?: string;
  ...
  appliedDiscounts?: RawAppliedDiscount[];
  refund?: { refundAmount?: number } | null;
  [key: string]: unknown;
}
...
interface RawAppliedDiscount {
  ...
  discount?: { guid?: string; name?: string };
...
const DAYPARTS: Array<{ name: string; hourStart: number; hourEnd: number }> = [
...
function classifyDaypart(isoTimestamp: string): string {
  const hour = new Date(isoTimestamp).getHours();
  for (const dp of DAYPARTS) {
    if (hour >= dp.hourStart && hour < dp.hourEnd) return dp.name;
  }
  return 'other';
}
...
export async function fetchDailySales(storeId: string, businessDate: string): Promise<DailySales> {
  ...
  const daypartMap = new Map<string, { orderCount: number; netSales: number }>();
  ...
  const channels: ChannelSales[] = Array.from(channelMap.values());

  const dayparts: DaypartSales[] = DAYPARTS.map(dp => {
...
export async function fetchOrders(
  ...
  const raw = await callTool('toast', 'toast_list_orders', args) as
    RawOrder[] | { orders: RawOrder[] };

  const orders: RawOrder[] = Array.isArray(raw) ? raw : (raw?.orders ?? []);
 ...
 */
export function computeItemMix(storeId: string, orders: RawOrder[], businessDate: string): ItemMix {
  ...
  const items: ItemMixEntry[] = Array.from(itemMap.values())
    .map(entry => ({
      ...entry,
...
export function computeDiscountSummary(
  ...
  const discountMap = new Map<string, { amount: number; count: number }>();
  ...
  const byType: DiscountEntry[] = Array.from(discountMap.entries())
    .map(([name, data]) => ({
      name,
 ...
 */
export function computeVoidsRefundsComps(
  storeId: string,
  orders: RawOrder[],
  netSales: number,
  businessDate: string,
...
export async function fetchLaborSummary(
  ...
  const raw = await callTool('toast', 'toast_list_shifts', { businessDate }) as
    { laborSummary?: RawLaborSummary; actual?: { timeEntries?: RawTimeEntry[] } } | null;
  ...
  const dayparts: DaypartLabor[] = [];
  const entries = raw.actual?.timeEntries ?? [];
  if (entries.length > 0) {
    const dpMap = new Map<string, { cost: number; hours: number; sales: number }>();
...
export async function fetchMenuItems(storeId: string): Promise<RawMenuItem[]> {
  logger.info('Fetching Toast menu items', { storeId });

  const raw = await callTool('toast', 'toast_menus', {}) as
    { menus: Array<{ groups?: Array<{ items?: RawMenuItem[] }> }> } | RawMenuItem[];
 ...
 * computed separately).
 */
export function detectDisabledItems(
  storeId: string,
  menuItems: RawMenuItem[],
  recentItemMix?: ItemMix,
): StockoutOrDisabledItem {
  ...
  for (const item of menuItems) {
    ...
    if (isVisible && !isHidden) continue;

    // Determine the reason
    let reason: 'disabled' | '86d' | 'hidden' | 'unknown' = 'unknown';
    ...
    if (recentItemMix) {
      const mixEntry = recentItemMix.items.find(
        m => m.itemGuid === item.guid,
      );
      if (mixEntry) {

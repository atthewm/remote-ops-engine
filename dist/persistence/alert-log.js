 ...
 * number of days.
 */
export function getRecentAlerts(storeId, days) {
    const store = ensureLoaded();
    const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
    return store.filter(alert => alert.storeId === storeId &&
        new Date(alert.createdAt).getTime() >= cutoff);
}
/**
 * Retrieves alerts for a given rule within the specified number of days.
 */
export function getAlertsByRule(ruleId, days) {
    const store = ensureLoaded();
    const cutoff = Date.now() - days * 24 * 60 * 60 * 1000;
    return store.filter(alert => alert.ruleId === ruleId &&
        new Date(alert.createdAt).getTime() >= cutoff);
}
/**
 ...
 */
export function getOpenAlerts(storeId) {
    const store = ensureLoaded();
    return store.filter(alert => alert.storeId === storeId &&
 ...
 */
export function archiveOldAlerts() {
    const store = ensureLoaded();

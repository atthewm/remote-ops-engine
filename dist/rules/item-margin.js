...
export class ItemMarginRule {
    ...
    async evaluate(storeId, config) {
        ...
        for (const menuItem of menuItems) {
            ...
            if (volume >= thresholds.highVolumeMinUnits &&
        ...
        // Determine severity
        let severity = 'yellow';
        const highVolumeBreaches = flaggedItems.filter(i => i.flags.includes('high_volume_margin_compression'));
        ...
        const fingerprint = generateFingerprint(RULE_ID, storeId, asOfDate, `flagged_${flaggedItems.length}_unmatched_${unmatchedTopSellers.length}`);
        const whatParts = [];
        ...
        if (flaggedItems.length > 0) {
            ...
            for (const item of top3) {
                whatParts.push(`${item.itemName}: price $${item.menuPrice.toFixed(2)}, cost $${item.estimatedCost.toFixed(2)}, margin ${pct(item.marginPercent)}, volume ${item.recentVolume} units.`);
            }
        }
 ...
 */
function findFuzzyMatch(normalizedName, productMap) {
...
function buildRecommendedAction(flaggedItems, unmatchedTopSellers, thresholds) {
    const actions = [];
    if (flaggedItems.length > 0) {
        actions.push(`Review pricing or vendor costs for ${flaggedItems.length} items below ${pct(thresholds.minMarginPercent)} margin.`);
        const highVolumeItems = flaggedItems.filter(i => i.recentVolume >= thresholds.highVolumeMinUnits);
        if (highVolumeItems.length > 0) {
            actions.push(`Priority: ${highVolumeItems.length} flagged items are high volume sellers. Address these first for maximum margin recovery.`);
        }
    }
    if (unmatchedTopSellers.length > 0) {
        actions.push(`Map ${unmatchedTopSellers.length} top selling items to MarginEdge products so margin can be tracked.`);
    }
    if (actions.length === 0) {
...
//# sourceMappingURL=item-margin.js.map
 ...
 */
async function fetchLaborData(storeId, businessDate) {
    try {
...
export class LaborRule {
    ...
    async evaluate(storeId, config) {
        ...
        if (laborData.overtimeHours > thresholds.overtimeHoursThreshold) {
            if (severity === 'green')
                severity = 'yellow';
            breaches.push(`Overtime hours (${laborData.overtimeHours.toFixed(1)}) exceed threshold of ${thresholds.overtimeHoursThreshold}`);
        }
        if (severity === 'green') {
        ...
        whatParts.push(`Total labor cost: $${laborData.totalLaborCost.toFixed(2)} across ${laborData.totalLaborHours.toFixed(1)} hours.`);
        if (netSales > 0) {
...
function buildDataUnavailableResult(storeId, businessDate, config) {
...
function buildRecommendedAction(breaches, laborData, thresholds) {
    const actions = [];

...
export class VendorPriceRule {
    ...
    async evaluate(storeId, config) {
        ...
        try {
            const recentInvoices = await fetchInvoiceStatus(storeId, fmt(recentStart), fmt(recentEnd));
            ...
            recentOrderIds = Array.from({ length: recentInvoices.totalInvoices }, (_, i) => `recent_${i}`);
        ...
        try {
            const previousInvoices = await fetchInvoiceStatus(storeId, fmt(previousStart), fmt(previousEnd));
            previousOrderIds = Array.from({ length: previousInvoices.totalInvoices }, (_, i) => `previous_${i}`);
        ...
        const spikes = priceChanges.changes.filter(c => c.changeDirection === 'up' && Math.abs(c.changePercent) >= thresholds.spikeThresholdPercent);
        if (spikes.length === 0) {
        ...
        const fingerprint = generateFingerprint(RULE_ID, storeId, asOfDate, `spikes_${spikes.length}`);
        const whatParts = [];
        whatParts.push(`Detected ${spikes.length} vendor price increases above the ${pct(thresholds.spikeThresholdPercent)} threshold.`);
        ...
        const decreases = priceChanges.changes.filter(c => c.changeDirection === 'down' && Math.abs(c.changePercent) >= thresholds.weekOverWeekThreshold);
...
function buildSpikeDetail(spike) {
    return (`${spike.productName} from ${spike.vendorName}: ` +
        `$${spike.previousPrice.toFixed(2)} to $${spike.currentPrice.toFixed(2)} ` +
        `(${pct(spike.changePercent)} increase).`);
}
//# sourceMappingURL=vendor-price.js.map
...
export class SalesPaceRule {
    ...
    async evaluate(storeId, config) {
        ...
        for (let weeksBack = 1; weeksBack <= trailingCount + 2; weeksBack++) {
            if (trailingSales.length >= trailingCount)
                break;
            ...
            if (trailingDate.getDay() !== currentDayOfWeek)
                continue;
            try {
        ...
        const fingerprint = generateFingerprint(RULE_ID, storeId, todayStr, `hour_${currentHour}`);
        const keyMetrics = {
        ...
        const whatParts = [];
        if (direction === 'below') {
            whatParts.push(`Sales are tracking ${pct(Math.abs(variance))} below the trailing ${trailingSales.length} week average for this day at ${currentHour}:00.`);
            whatParts.push(`Current: $${currentSales.toFixed(2)}. Expected at this point: $${round2(expectedPaceNow).toFixed(2)}.`);
            whatParts.push(`Trailing full day average: $${round2(trailingAvgFullDay).toFixed(2)}.`);
        }
        else {
            whatParts.push(`Sales are tracking ${pct(Math.abs(variance))} above the trailing ${trailingSales.length} week average for this day at ${currentHour}:00.`);
            whatParts.push(`Current: $${currentSales.toFixed(2)}. Expected at this point: $${round2(expectedPaceNow).toFixed(2)}.`);
        }
...
//# sourceMappingURL=sales-pace.js.map
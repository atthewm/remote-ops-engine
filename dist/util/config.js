...
export function getStore(config, storeId) {
...
export function getOwnerForDomain(config, domain) {
    return config.owners.defaultOwnerByDomain[domain] ?? 'unassigned';
}
export function getOwner(config, ownerId) {
    return config.owners.owners.find(o => o.id === ownerId) ?? null;
}
export function getThreshold(config, path) {
    const parts = path.split('.');
    let current = config.rules.thresholds;
